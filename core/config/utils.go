...
func PrepareVars(c *clabcore.CLab) map[string]*NodeConfig {
	...
	topoc := NewConfigTopo(c)

	for name, node := range topoc.Nodes {
		res[name] = &NodeConfig{
			TargetNode:  c.Nodes[name].Config(),
			Vars:        make(map[string]any),
			Credentials: c.Reg.Kind(node.Kind).GetCredentials().Slice(),
		}
	}

	// populate all the node vars
	for _, node := range res {
		nodeCfg := node.TargetNode
		node.Vars[vkNodeName] = nodeCfg.ShortName
		node.Vars[vkKind] = nodeCfg.Kind
		node.Vars[vkManagementIPv4] = nodeCfg.MgmtIPv4Address
		node.Vars[vkManagementIPv6] = nodeCfg.MgmtIPv6Address
		node.Vars[vkType] = nodeCfg.NodeType

		// Init array for this node
		for key, val := range nodeCfg.Config.Vars {
			if key == vkNodes || key == vkNodeName {
				log.Warnf("the variable %s on %s will be ignored, it hides other nodes", vkNodes, nodeCfg.ShortName)
				continue
			}
			//if node
			node.Vars[key] = val
		...
		// Create link array
		node.Vars[vkLinks] = []any{}

		// Ensure role or Kind
		if _, ok := node.Vars[vkRole]; !ok {
			node.Vars[vkRole] = nodeCfg.Kind
		}

	}

	// prepare all links
	for lIdx, link := range topoc.Links {
		varsA := make(Dict)
		varsB := make(Dict)
		err := prepareLinkVars(&link, varsA, varsB, res)
		if err != nil {
			log.Errorf("cannot prepare link vars for %d. %s: %s", lIdx, link, err)
		}
		res[link.Source].Vars[vkLinks] = append(res[link.Source].Vars[vkLinks].([]any), varsA)
		res[link.Target].Vars[vkLinks] = append(res[link.Target].Vars[vkLinks].([]any), varsB)
	}
...
func prepareLinkVars(link *ConfigLink, varsA, varsB Dict, nodes map[string]*NodeConfig) error {
	// Add a Dict for the far-end link vars and the far-end node name
	varsA[vkFarEnd] = Dict{vkNodeName: link.Target}
	varsB[vkFarEnd] = Dict{vkNodeName: link.Source}
	...
	add := map[string]func(link *ConfigLink, sourceVars map[string]any, targetVars map[string]any) (string, string, error){
		vkLinkIP:   linkIP,
		vkLinkName: linkName,
	}
	...
	for k, f := range add {
		...
		a, b, err := f(link, nodes[link.Source].Vars, nodes[link.Target].Vars)
		if err != nil {
...
func linkName(link *ConfigLink, sourceVars map[string]any, targetVars map[string]any) (string, string, error) { //nolint: unparam
	var linkNo string
	if v, ok := link.Vars[vkLinkNum]; ok {
		linkNo = fmt.Sprintf("_%v", v)
	}
	return fmt.Sprintf("to_%s%s", link.Target, linkNo),
		fmt.Sprintf("to_%s%s", link.Source, linkNo), nil
}

// Calculate link IP from the system IPs at both ends.
func linkIP(link *ConfigLink, sourceVars map[string]any, targetVars map[string]any) (string, string, error) {
	var ipA netip.Prefix
	var err error
	//
	_, okA := sourceVars[vkSystemIP]
	_, okB := targetVars[vkSystemIP]
	if okA != okB {
		log.Warnf("to auto-generate link IPs, a %s variable is required on all nodes [link:%s]", vkSystemIP, link.String())
	}
	if !okA || !okB {
		return "", "", nil
	}
	sysA, err := netip.ParsePrefix(fmt.Sprintf("%v", sourceVars[vkSystemIP]))
	if err != nil {
		return "", "", fmt.Errorf("no 'ip' on link & the '%s' of %s: %s", vkSystemIP, link.Source, err)
	}
	sysB, err := netip.ParsePrefix(fmt.Sprintf("%v", targetVars[vkSystemIP]))
	if err != nil {
		return "", "", fmt.Errorf("no 'ip' on link & the '%s' of %s: %s", vkSystemIP, link.Target, err)
	}
...
func GetTemplateNamesInDirs(dirs []fs.FS) ([]string, error) {
	...
	if len(tnames) == 0 {
		return nil, fmt.Errorf("no templates files were found in specified paths: %v", dirs)
	}
	return tnames, nil
}
