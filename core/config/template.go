...
func LoadTemplates(tmpl *template.Template, role string, TemplatePaths []string) error {
...
func RenderAll(allnodes map[string]*NodeConfig, templateList []string, templatePaths []string) error {
	...
	if len(templateList) == 0 {
		var err error
		templateList, err = GetTemplateNamesInDirs(TemplateFS)
		if err != nil {
			return err
		}
		log.Infof("No template names specified (-l) using: %s", strings.Join(templateList, ", "))
	}

	tmpl := template.New("").Funcs(utils.AllFuncs())

	for _, nc := range allnodes {
		for _, baseN := range templateList {
			tmplN := fmt.Sprintf("%s__%s.tmpl", baseN, nc.Vars[vkRole])
			log.Debugf("Looking up template %v", tmplN)
			if l := tmpl.Lookup(tmplN); l == nil {
				err := LoadTemplates(tmpl, fmt.Sprintf("%s", nc.Vars[vkRole]), templatePaths)
				if err != nil {
					return err
				}
				l = tmpl.Lookup(tmplN)
